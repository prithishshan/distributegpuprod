...
// import debugBVHShaderSrc from "./shader-bts/debug-bvh";
// import presentShaderSrc from "./shader-bts/present";
// import raytracerShaderSrc from "./shader-bts/raytracer";
// import CameraShaderChunk from "./shader-bts/utils/camera";
// import CommonShaderChunk from "./shader-bts/utils/common";
...
// 	if($frameCounter) $frameCounter.textContent = frameCounter.toString();
// 	if($timeExpired) $timeExpired.textContent = dayjs.duration(timeExpiredMs).format("mm:ss");
// 	const progresPercent = (frameCounter / guiSettings["Max Samples"]) * 100;
// 	if($progress) $progress.style.width = `${progresPercent}%`;
// 	if ($progresPercent){
// 		$progresPercent.className = progresPercent < 5 ? "docked-left" : "docked-right";
...
// 		resetRender();
// 	});
// 	gui.add(guiSettings, "Use Phong Shading").onChange((v: boolean) => {
// 		flatShading = v ? 0 : 1;
// 		resetRender();
// 	});
// 	gui.add(guiSettings, "Crystal Suzanne").onChange((v: boolean) => {
// 		scene.isSuzanneGlass = v;
// 		resetRender();
// 	});
// 	gui.add(guiSettings, "Ray Bounces Count", 1, 16, 1).onChange((v: number) => {
// 		resetRender();
// 		maxBounces = v;
// 	});
// 	gui.add(guiSettings, "Max Samples", 1, 10000, 5).onChange((v: number) => {
// 		resetRender();
// 	});
// }
